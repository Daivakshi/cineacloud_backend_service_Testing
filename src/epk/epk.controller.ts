...
export class EpkController {
    ...
    async createEpk(@Req() req: Request, @Res() res: Response, @Body() body: CreateEpkDto) {
        try {
            ...
            if(epkId && epkId.toString().length == 24) {
                ...
                let ifepKExsist = await this.epk.findOneEpk({ epkName, userId: req.user._id })
    ...
    async downloadEpk(@Req() req: Request, @Res() res: Response, @Query('epkId') epkId: string) {
        try {
            ...
            let templates = await this.template.getTemplateByEpkId({
            ...
            for (let template of templates) {
                template["url"] = `https://demo-cineacloud.s3.us-east-2.amazonaws.com/${template.preview}`//await this.s3.s3GetSignedURL(template.preview)
            }
